...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
    <h1 class="h2" style="background: var(--gradient-bg); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">
        <i class="fas fa-arrow-up me-2"></i>Tambah Stok Keluar
    </h1>
</div>
        ...
        <form action="{{ route('stock-outs.store') }}" method="POST">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        @error('quantity')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="form-text text-muted">Stok tersedia: <span id="available-stock">-</span></small>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('note') is-invalid @enderror" 
                          id="note" name="note" rows="3" placeholder="Keterangan stok keluar...">{{ old('note') }}</textarea>
                @error('note')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
...
<script>
    document.getElementById('product_id').addEventListener('change', function() {
        ...
        document.getElementById('available-stock').textContent = stock + ' ' + 'pcs';
    });
</script>
@endsection